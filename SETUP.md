...
Each product carries two prices and a stock count:

| Field | Meaning |
|---|---|
| **Prodajna cena** | what the customer pays |
| **Nabavna cena** | what the salon pays the supplier (optional) |
| **Zaloga** | how many are on the shelf right now |

The selling price is required; the cost is optional but worth filling in, since
it is what produces the margin column and the cost totals. It is also used as
the default when recording a delivery, so the usual case is one click.
...
Because the history is deleted after a year, a worker can keep a copy. Two
buttons on **Izdelki → Mesečno poročilo**:

- **PDF — 2026-08** — the chosen month, every product, with a totals row.
- **PDF — vsi meseci** — every month still kept, one section each.

The product page also has **PDF zgodovine**: that one product's months plus
every individual movement, with the notes and who recorded them.

Add `&inline=1` to any of those links to view the report in the browser instead
of downloading it, which is handy for a quick check before saving.

The PDFs are generated by the app itself with no external tool or service, so
they work on a server with no internet access, and there is nothing extra to
...
- [ ] products have a Nabavna cena where you know it, so the margin columns
      and the cost totals are not empty
